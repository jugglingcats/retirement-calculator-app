import { AssetBalances, PersonIncomeData, SplitShortfallResult, AssetDrawdownResult } from "@/lib/types"
...
export class TaxOptimizedStrategy extends BaseDrawdownStrategy {
    public withdrawFromAssets(
        ...
        const roomBeforeHigherRate = Math.max(0, this.taxSettings.higherRateThreshold - currentTaxableIncome)
    ...
    executeSingle(assets: AssetBalances, shortfall: number, income: PersonIncomeData): SplitShortfallResult {
        const taxableIncome = this.getTaxableIncome(income)

        const result = this.withdrawFromAssets(assets, shortfall, taxableIncome)

        const primaryTax = this.calcTax(taxableIncome + result.taxableWithdrawn)

        return {
            primaryTax,
            spouseTax: 0,
            primaryWithdrawn: shortfall - result.remaining,
            spouseWithdrawn: 0
        }
    }
